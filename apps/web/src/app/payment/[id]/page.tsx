...
const PaymentPage: React.FC = () => {
    ...
    const fetchEventDetails = async () => {
      try {
        const response = await fetch(
          `${serverHost}api/event/eventdetail/${eventId?.id}`
        );
        if (!response.ok) {
  ...
  const handleSubmit = () => {
    const createPayment = async (userData: any, accessToken: string) => {
      const r = await fetch(serverHost + "api/transaction/create", {
        method: "POST",
    ...
    <div className="min-h-screen bg-gray-100 flex justify-center items-center p-4">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        ...
        <div className="space-y-4">
          <div className="flex items-center bg-gray-200 p-3 rounded-lg gap-4">
            {/* Gambar Event */}
            <img
              src={
